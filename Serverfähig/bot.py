...
def _fingerprint(t: str) -> str:
    return f"{t[:6]}…{t[-6:]}" if len(t) >= 12 else "<leer>"

TOKEN = "DEIN-TOKEN" # Token hier
print(f"Token-Fingerprint: {_fingerprint(TOKEN)}")
...
@purger_select.error
async def purger_select_error(interaction: discord.Interaction, error: Exception):
    if isinstance(error, app_commands.MissingPermissions):
        return await _safe_ephemeral(interaction, "Dir fehlt die Berechtigung: Server verwalten.")
    raise error

@purger_clear.error
async def purger_clear_error(interaction: discord.Interaction, error: Exception):
    if isinstance(error, app_commands.MissingPermissions):
        return await _safe_ephemeral(interaction, "Dir fehlt die Berechtigung: Server verwalten.")
    raise error

@purger_run.error
async def purger_run_error(interaction: discord.Interaction, error: Exception):
    if isinstance(error, app_commands.MissingPermissions):
        return await _safe_ephemeral(interaction, "Dir fehlt die Berechtigung: Nachrichten verwalten.")
    raise error

@purger_list.error
async def purger_list_error(interaction: discord.Interaction, error: Exception):
    if isinstance(error, app_commands.MissingPermissions):
        # Sollte nicht vorkommen, da list keine Checks hat; nur für Sicherheit
        return await _safe_ephemeral(interaction, "Dir fehlen Berechtigungen.")
    raise error
